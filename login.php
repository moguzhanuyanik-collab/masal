...
app_session_start();
if (authenticated_student_id() !== null) {
...
try {
    ...
    if (!auth_accounts_ready($pdo)) {
        throw new RuntimeException('Henüz giriş yapılabilecek öğrenci hesabı bulunmuyor.');
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare('SELECT id, sifre_hash FROM ogrenciler WHERE email=? AND aktif=1 LIMIT 1');
        $stmt->execute([$email]);
        $student = $stmt->fetch();

        if (!is_array($student) || empty($student['sifre_hash']) || !password_verify($password, (string)$student['sifre_hash'])) {
            usleep(250000);
        ...
        try {
            $pdo->prepare('DELETE FROM ogrenci_oturum_tokenlari WHERE son_kullanma_tarihi<=NOW()')->execute();
        } catch (Throwable) {}

        if ($remember) {
            clear_remember_cookie($pdo);
            remember_student($pdo, $studentId);
        }
...
<title>Öğrenci Girişi — İlkAdım</title>
...
<p>Öğrenme dünyana devam etmek için giriş yap.</p>
</section>
