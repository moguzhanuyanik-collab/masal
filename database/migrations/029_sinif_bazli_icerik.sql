...
SET @has_ilerleme_fk_index=(SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='ogrenci_ilerleme' AND index_name='ix_ogrenci_ilerleme_ogrenci');
SET @sql=IF(@has_ilerleme_fk_index=0,'ALTER TABLE ogrenci_ilerleme ADD KEY ix_ogrenci_ilerleme_ogrenci (ogrenci_id)','SET @ilkadim_noop=1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @ilerleme_index_cols=(SELECT GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='ogrenci_ilerleme' AND index_name='uk_ogrenci_ilerleme');
SET @sql=IF(@ilerleme_index_cols IS NOT NULL AND @ilerleme_index_cols<>'ogrenci_id,sinif_seviyesi,ders_kodu,modul_indeksi','ALTER TABLE ogrenci_ilerleme DROP INDEX uk_ogrenci_ilerleme','SET @ilkadim_noop=1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @has_new_ilerleme_unique=(SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='ogrenci_ilerleme' AND index_name='uk_ogrenci_ilerleme');
SET @sql=IF(@has_new_ilerleme_unique=0,'ALTER TABLE ogrenci_ilerleme ADD UNIQUE KEY uk_ogrenci_ilerleme (ogrenci_id,sinif_seviyesi,ders_kodu,modul_indeksi)','SET @ilkadim_noop=1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
