...
function normalized_badges(PDO $pdo,array $state):array{
    $steps=[];foreach(($state['steps']??[]) as $k)if(preg_match('/^([a-z0-9_-]+)-(\\d+)$/i',(string)$k,$m))$steps[$m[1]][(int)$m[2]]=true;
    $counts=[];foreach($pdo->query("SELECT d.kod,COUNT(m.id) modul_sayisi FROM dersler d LEFT JOIN ders_modulleri m ON m.ders_id=d.id AND m.aktif=1 WHERE d.aktif=1 GROUP BY d.id,d.kod")?:[] as $r)$counts[(string)$r['kod']]=(int)$r['modul_sayisi'];
...
function normalized_sync(PDO $pdo,int $studentId,array $input):void{
    ...
    foreach(['ogrenci_ilerleme','ogrenci_cevaplari','favoriler','oyun_tamamlamalari','okuma_kayitlari','calisma_oturumlari','gunluk_kullanim','ogrenci_rozetleri','gunluk_gorevler'] as $t){
        if(!normalized_table_exists($pdo,$t))continue;$s=$pdo->prepare("DELETE FROM {$t} WHERE ogrenci_id=?");$s->execute([$studentId]);
    }
    $s=$pdo->prepare('INSERT INTO ogrenci_ilerleme (ogrenci_id,ders_kodu,modul_indeksi,tamamlandi) VALUES (?,?,?,1)');
    foreach($state['steps'] as $k)if(preg_match('/^([a-z0-9_-]+)-(\\d+)$/i',(string)$k,$m))$s->execute([$studentId,$m[1],(int)$m[2]]);
    ...
    $s=$pdo->prepare('INSERT INTO ogrenci_cevaplari (ogrenci_id,ders_kodu,soru_anahtari,secilen_cevap,dogru,sure_ms,cevap_tarihi) VALUES (?,?,?,?,?,?,?)');
    foreach(array_reverse($state['attempts']) as $a){
        ...
        $s->execute([$studentId,mb_substr((string)$a['lesson'],0,50),mb_substr((string)($a['index']??''),0,190),$sel===false?null:$sel,($a['correct']??false)===true?1:0,max(0,min(3600000,(int)($a['ms']??0))),normalized_datetime($a['at']??null)]);
    }
    ...
    foreach(normalized_badges($pdo,$state) as $c){$find->execute([$c]);$id=$find->fetchColumn();if($id)$ins->execute([$studentId,(int)$id]);}
    $daily=[];
...
function normalized_summary(PDO $pdo,int $studentId):array{
    ...
    $q=['completed_steps'=>'SELECT COUNT(*) FROM ogrenci_ilerleme WHERE ogrenci_id=? AND tamamlandi=1','favorites'=>'SELECT COUNT(*) FROM favoriler WHERE ogrenci_id=?','answers'=>'SELECT COUNT(*) FROM ogrenci_cevaplari WHERE ogrenci_id=?','correct_answers'=>'SELECT COUNT(*) FROM ogrenci_cevaplari WHERE ogrenci_id=? AND dogru=1','games'=>'SELECT COUNT(*) FROM oyun_tamamlamalari WHERE ogrenci_id=?','readings'=>'SELECT COUNT(*) FROM okuma_kayitlari WHERE ogrenci_id=?','badges'=>'SELECT COUNT(*) FROM ogrenci_rozetleri WHERE ogrenci_id=?'];
    foreach($q as $k=>$sql){$s=$pdo->prepare($sql);$s->execute([$studentId]);$out[$k]=(int)$s->fetchColumn();}
    $s=$pdo->prepare('SELECT toplam_yildiz FROM ogrenci_yildizlari WHERE ogrenci_id=? LIMIT 1');$s->execute([$studentId]);$out['stars']=(int)($s->fetchColumn()?:0);
    return $out;
}
