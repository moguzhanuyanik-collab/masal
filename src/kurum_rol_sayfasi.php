...
<?php if($kyRole==='ogrenci'):?><label>Kademe</label><input class="role-input" value="Temel Eğitim" readonly><label>Sınıf</label><select class="role-input" name="sinif_seviyesi" required><?php for($g=1;$g<=8;$g++):?><option value="<?=$g?>"><?=$g?>. sınıf</option><?php endfor;?></select><?php endif;?>
...
<?php else:foreach($members as $m):?><div class="role-row"><span><?=ky_h($kyIcon)?></span><div><strong><?=ky_h((string)($m['ad']?:$m['ad_soyad']))?></strong><small><?=ky_h((string)$m['email'])?><?=$kyRole==='ogrenci'?' · Temel Eğitim · '.min(8,max(1,(int)($m['sinif_seviyesi']??1))).'. sınıf':''?></small></div><span class="role-pill <?=((int)$m['aktif']===1?'ok':'off')?>"><?=((int)$m['aktif']===1?'Aktif':'Pasif')?></span></div><?php endforeach;endif;?>
</div></section>
</main>
